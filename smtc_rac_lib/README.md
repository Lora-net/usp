...
graph TD
    ...
    conf[["**Handle Configuration**<br/>smtc_rac_request_radio_access()<br/>smtc_rac_get_context()"]]
...
```bash
rm -Rf build/ ; cmake -L -S examples  -B build -DAPP=PERIODICAL_UPLINK -DCMAKE_BUILD_TYPE=MinSizeRel -DBOARD=NUCLEO_L476 -DRAC_RADIO=lr2021 -DLBM_RADIO=lr2021 -G Ninja; cmake --build build --target periodical_uplink

or

rm -Rf build/ ; cmake -L -S examples  -B build -DAPP=RTTOF -DCMAKE_BUILD_TYPE=MinSizeRel -DBOARD=NUCLEO_L476 -DRAC_RADIO=lr2021
-DLBM_RADIO=lr2021 -DRANGING_DEVICE_MODE=SUBORDINATE -DCONTINUOUS_RANGING=false -G Ninja; cmake --build build rttof_subordinate
```
