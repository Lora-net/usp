...
By default this configuration is not retained in memory when entering sleep mode. The function `lr20xx_workarounds_bluetooth_le_phy_coded_frequency_drift_store_retention_mem` allows to store the Bluetooth LE frequency drift workaround in a retention memory slot.
