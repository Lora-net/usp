...
{
    ralf_params_lora_cad_t ralf_cad_params;
...
typedef struct smtc_rac_data_result_s
{
    ...
    int8_t              snr_result;               /*!< Signal-to-Noise Ratio (SNR) value. */
...
{
    uint32_t              start_time_ms;              /*!< Scheduled start time in milliseconds (absolute time). */
    ...
    void ( *callback_pre_radio_transaction )( void ); /*!< Callback called just before the radio transaction. */
    void ( *callback_post_radio_transaction )(
        rp_status_t status ); /*!< Callback called just after the radio transaction. */
